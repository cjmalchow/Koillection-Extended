...
backup_color = app_settings.get("backup_color", "#FF5722")

if not box_config and not st.session_state.hide_instructions:
    st.session_state.hide_instructions = False
elif box_config and "hide_instructions_toggled" not in st.session_state:
    st.session_state.hide_instructions = True
    st.session_state.hide_instructions_toggled = True
...
for _, row in df.iterrows():
    ...
    if iid not in items_data:
        items_data[iid] = {"id": iid, "name": str(row['name']), "image": row['image'], "primary_location": "", "other_locations": "", "fields": {}}
    ...
    if pd.notna(lbl) and pd.notna(val):
        ...
        if lbl_str.lower() == 'location':
            if items_data[iid]["primary_location"]:
                ...
                items_data[iid]["primary_location"] = val_str
        elif lbl_str.lower() == 'other location(s)':
            if items_data[iid]["other_locations"]:
                items_data[iid]["other_locations"] += f", {val_str}"
            else:
                items_data[iid]["other_locations"] = val_str
        else:
...
def update_item_field(item_id, label, new_value):
    conn = st.connection("koillection_db", type="sql", url=get_db_url())
    ...
    with conn.session as s:
        check_query = text("""
            ...
            LIMIT 1
        """)
        result = s.execute(check_query, {"id": item_id, "label": label}).fetchone()
        
        if result:
            update_query = text("""
                UPDATE koi_datum 
                SET value = :val, updated_at = NOW()
                WHERE id = CAST(:datum_id AS text)
            """)
...
def assign_dialog(box_name, col, row):
    slot_name = f"{box_name}-{col}{row}"
    st.markdown(f"Assign a polish to **{slot_name}**")
    
    unassigned = []
    for iid, data in items_data.items():
        loc = data["primary_location"].strip()
        if not loc or loc in other_containers:
            unassigned.append((iid, data["name"], loc))
    
    if not unassigned:
        st.warning("No unassigned polishes found! All polishes are currently in structured boxes.")
        return
        
    options = {iid: f"{name} (Current: {loc or 'None'})" for iid, name, loc in unassigned}
    selected_id = st.selectbox("Select Polish", options=list(options.keys()), format_func=lambda x: options[x])
    
    if st.button("Assign to Slot", type="primary", use_container_width=True):
        with st.spinner("Updating database..."):
            update_item_field(selected_id, "Location", slot_name)
            st.cache_data.clear()
            time.sleep(0.5)
        st.rerun()
...
def grid_text_dialog():
    ...
    st.markdown("### Backup Polish Settings")
    new_backup_color = st.color_picker("Backup Polish Text Color", value=backup_color, help="Polishes from 'Other Location(s)' will use this color.")
    ...
    if st.button("Save Display Settings", type="primary", use_container_width=True):
        with st.spinner("Saving display settings..."):
            ...
            app_settings["backup_color"] = new_backup_color
            save_settings(app_settings)
            time.sleep(0.5)
        st.rerun()
...
def sticker_customization_dialog():
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    new_sticker_backup_color = st.color_picker("Backup Polish Text Color", value=sticker_settings.get("backup_color", "#FF0000"), help="Color for polishes stored in 'Other Location(s)'.")
    ...
    if st.button("Save Sticker Settings", type="primary", use_container_width=True):
        with st.spinner("Saving sticker settings..."):
            app_settings["sticker_settings"] = {
                ...
                "backup_color": new_sticker_backup_color
            }
            save_settings(app_settings)
            time.sleep(0.5)
...
if not st.session_state.hide_instructions:
    with st.expander("ℹ️ How to use the Locator Guide", expanded=True):
        ...
        **Step 2: Configure your Boxes**
        ...
        ### 🚀 Using the Locator
        * **Click-to-Assign:** In the Visual Grid view, click the ➕ button on any empty slot to instantly assign an unboxed polish to that location!
        * **Spreadsheet View:** Use the new Spreadsheet View to rapidly type in locations and bulk-save them to the database.
...
def assign_to_boxes(loc_string, item, is_backup):
    if not loc_string: return
    locations = [l.strip() for l in loc_string.split(',')]
        ...
        if match:
            ...
            if parsed_box_name in box_name_map:
                ...
                if row_num in boxes[actual_box_name] and col in boxes[actual_box_name][row_num]:
                    boxes[actual_box_name][row_num][col] = {"item": item, "is_backup": is_backup}
                else:
                    if loc not in other_containers: other_containers[loc] = []
                    other_containers[loc].append({"item": item, "is_backup": is_backup})
            else:
                if loc not in other_containers: other_containers[loc] = []
                other_containers[loc].append({"item": item, "is_backup": is_backup})
        else:
...
for item in items_data.values():
    assign_to_boxes(item["primary_location"], item, is_backup=False)
    assign_to_boxes(item["other_locations"], item, is_backup=True)
...
def get_next_available_slot():
    ...
    for b_name in sorted(boxes.keys(), key=natural_keys):
        dims = box_config[b_name]
        cols = [chr(65 + i) for i in range(dims['cols'])]
        rows = dims['rows']
        unusable = dims.get("unusable", [])
        
        for r in range(1, rows + 1):
            for c in cols:
                if f"{c}{r}" in unusable:
                    continue
                if boxes[b_name][r][c] is None:
                    return f"{b_name}-{c}{r}"
    return "No empty slots!"

# --- UI GENERATION ---
view_mode = st.radio("Select Grid View:", ["Visual Grid (Images)", "Sticker Grid (Printable)", "Spreadsheet View (Bulk Edit)"], horizontal=True, key="view_mode")

html_size_map = {"Small": "9px", "Medium": "11px", "Large": "14px", "Auto-Fit": "12px"}
print_size_map = {"Small": 20, "Medium": 28, "Large": 36}

def render_interactive_grid(box_num, box_data):
    ...
    for i, c in enumerate(cols):
        with header_cols[i+1]:
            st.markdown(f"<div style='text-align:center; font-weight:bold; padding-bottom: 10px;'>{c}</div>", unsafe_allow_html=True)
    
    for r in range(1, rows + 1):
        row_cols = st.columns(len(cols) + 1)
        with row_cols[0]:
            st.markdown(f"<div style='text-align:center; font-weight:bold; margin-top: 60px;'>{r}</div>", unsafe_allow_html=True)
        
        for i, c in enumerate(cols):
            with row_cols[i+1]:
                cell_id = f"{c}{r}"
                slot_data = box_data[r][c]
                
                if cell_id in unusable_cells:
                    st.markdown("<div style='background-color: black; height: 140px; border: 1px solid #555; border-radius: 5px;'></div>", unsafe_allow_html=True)
                elif slot_data is None:
                    st.markdown("<div style='height: 45px;'></div>", unsafe_allow_html=True)
                    if st.button("➕", key=f"btn_{box_num}_{cell_id}", help=f"Assign polish to {box_num}-{cell_id}", use_container_width=True):
                        assign_dialog(box_num, c, r)
                else:
                    item = slot_data["item"]
                    is_backup = slot_data["is_backup"]
                    
                    img_b64, mime = get_image_base64(item["image"])
                    img_tag = f"<img src='data:{mime};base64,{img_b64}' style='width: 100%; height: 100%; object-fit: cover; position: absolute; top: 0; left: 0; border-radius: 5px;'>" if img_b64 else ""
                    
                    text_color = backup_color if is_backup else "white"
                    ...
                    for f in display_fields:
                        ...
                        if val:
                            ...
                            if f == "Name":
                                koillection_url = os.getenv("KOILLECTION_URL", "http://localhost:8081")
                                val = f"<a href='{koillection_url}/items/{item['id']}' target='_blank' style='color: inherit; text-decoration: inherit;'>{val}</a>"
                                
                            text_html += f"<div style='font-size: {fz}; font-weight:{fw}; font-style:{fs}; text-decoration:{td}; text-align:{ta}; color:{text_color}; text-shadow: 1px 1px 2px black; width: 100%;'>{val}</div>"
                    
                    # Append Primary Location if it's a backup
                    if is_backup and item["primary_location"]:
                        text_html += f"<div style='font-size: 10px; color:{text_color}; text-shadow: 1px 1px 2px black; text-align:center; margin-top: 2px;'>(Primary: {item['primary_location']})</div>"
                    
                    card_html = f"<div style='border: 1px solid #555; border-radius: 5px; height: 140px; position: relative; padding: 0; background: #222; overflow: hidden;'>{img_tag}<div style='position: relative; z-index: 1; padding: 5px; display: flex; flex-direction: column; align-items: center;'><div style='background: rgba(0,0,0,0.7); display: inline-block; padding: 3px 6px; border-radius: 4px; width: 90%;'>{text_html}</div></div></div>"
...
def get_sticker_html_grid(box_num, box_data):
    ...
    base_text_color = ss.get("text_color", "#000000")
    sticker_backup_color = ss.get("backup_color", "#FF0000")
    ...
    html += f"<table style='width: 100%; border-collapse: collapse; table-layout: fixed; color: {base_text_color}; font-family: Arial, sans-serif;'>"
    ...
    for r in range(1, rows + 1):
        ...
        for c in cols:
            ...
            if cell_id in unusable_cells:
                ...
                html += f"<div style='font-weight: bold; font-size: 12px; color: {base_text_color};'>{c}{r}</div>"
                
                if slot_data:
                    item = slot_data["item"]
                    is_backup = slot_data["is_backup"]
                    current_text_color = sticker_backup_color if is_backup else base_text_color
                    
                    for f in display_fields:
                        val = item['name'] if f == "Name" else item['fields'].get(f, "")
                        if val:
                            if f == "Name" and is_backup and item["primary_location"]:
                                val += f" (Primary: {item['primary_location']})"
                            ...
                            html += f"<div style='font-size: {fz}; margin-top: 3px; word-wrap: break-word; font-weight:{fw}; font-style:{fs}; text-decoration:{td}; text-align:{ta}; color: {current_text_color};'>{val}</div>"
...
def get_sticker_image_bytes(box_num, box_data):
    ...
    for r in range(1, rows + 1):
        for c_idx, c in enumerate(cols):
            ...
            draw.text((x0 + 10, y0 + 10), cell_id, font=font_cell_id, fill=base_text_color)
...
if not box_config:
    ...
    if st.session_state.view_mode == "Spreadsheet View (Bulk Edit)":
        st.markdown("### 📝 Bulk Edit Locations")
        st.info(f"💡 **Next Available Slot:** `{get_next_available_slot()}`")
        
        edit_df = pd.DataFrame([
            {"ID": iid, "Name": data["name"], "Location": data["primary_location"], "Other Locations": data["other_locations"]}
            for iid, data in items_data.items()
        ])
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                "ID": None,
                "Name": st.column_config.TextColumn("Polish Name", disabled=True),
                "Location": st.column_config.TextColumn("Location (e.g., 1-A1)"),
                "Other Locations": st.column_config.TextColumn("Other Locations (Backups)")
            },
        ...
        if st.button("💾 Save All Changes", type="primary"):
            with st.spinner("Saving to database..."):
                ...
                for index, row in edited_df.iterrows():
                    old_loc = edit_df.loc[index, "Location"]
                    new_loc = row["Location"]
                    if old_loc != new_loc:
                        update_item_field(row["ID"], "Location", new_loc)
                        changes += 1
                        
                    old_other = edit_df.loc[index, "Other Locations"]
                    new_other = row["Other Locations"]
                    if old_other != new_other:
                        update_item_field(row["ID"], "Other Location(s)", new_other)
                        changes += 1
                
                if changes > 0:
                    st.cache_data.clear()
                    st.success(f"Successfully updated {changes} locations!")
                    time.sleep(1)
                    st.rerun()
                else:
            ...
            for i, b in enumerate(box_keys):
                with tabs[i]:
                    if st.session_state.view_mode == "Sticker Grid (Printable)":
                        ...
                        render_interactive_grid(b, boxes[b])

            for i, c in enumerate(sorted(list(other_containers.keys()))):
                with tabs[i + len(box_keys)]:
                    st.subheader(f"Location: {c}")
                    st.table(pd.DataFrame({"Polish Name": [slot["item"]['name'] for slot in other_containers[c]]}))